...
// ── Init map (reinit on mobile/desktop switch) ──────────────────────────────
useEffect(() => {
if (!leafletReady || !mapRef.current) return;
// Destroy existing instance if present (layout switch)
if (mapInst.current) {
...
<style>{`* { box-sizing: border-box; } input, button, select { font-family: inherit; } ::-webkit-scrollbar { width: 4px; } ::-webkit-scrollbar-track { background: #0a0e1a; } ::-webkit-scrollbar-thumb { background: #1a2535; border-radius: 2px; } @keyframes pulse { 0%,100%{opacity:1} 50%{opacity:0.3} } @keyframes spin { from{transform:rotate(0deg)} to{transform:rotate(360deg)} } @keyframes fadeIn { from{opacity:0;transform:translateY(4px)} to{opacity:1;transform:translateY(0)} } @keyframes modalIn { from{opacity:0;transform:scale(0.96)} to{opacity:1;transform:scale(1)} } @keyframes sheetUp { from{transform:translateY(100%)} to{transform:translateY(0)} } .agency-row:hover { background: #0f1e30 !important; border-color: #1d3557 !important; } .btn-ghost:hover { background: #0d1421 !important; border-color: #1d3557 !important; } .btn-primary:hover { background: #c0303c !important; } html, body, #root { height: 100%; margin: 0; }`}</style>
...
```
  ...
  {disclaimerAccepted && (
    <div style={{
      background: "#0a0f1a", borderBottom: `1px solid ${css.border}`,
      ...
      {[
        ...
        <button
          ...
          style={{
            ...
            color, padding: "4px 10px", cursor: "pointer", fontSize: 10,
            letterSpacing: "0.1em", display: "flex", alignItems: "center", gap: 5, position: "relative"
          }}
        >
          📋 {label}
          <span id={`copied-${label}`} style={{
            position: "absolute", top: -22, left: "50%", transform: "translateX(-50%)",
            background: css.card, border: `1px solid ${css.border}`, borderRadius: 3,
            padding: "2px 7px", fontSize: 9, color: css.green, whiteSpace: "nowrap",
            opacity: 0, transition: "opacity 0.2s", pointerEvents: "none"
          }}>Copied!</span>
        </button>
      ))}
      <a href="https://ko-fi.com/emberdrift" target="_blank" rel="noopener noreferrer" style={{
        background: "#ff5e5b18", border: "1px solid #ff5e5b44", borderRadius: 4,
        color: "#ff5e5b", padding: "4px 10px", fontSize: 10,
        textDecoration: "none", letterSpacing: "0.1em", whiteSpace: "nowrap"
      }}>☕ Ko-fi</a>
    </div>
  )}
  ...
  <div style={{
    ...
    background: "linear-gradient(90deg, #080c14 0%, #0a0f1a 100%)",
    flexShrink: 0, position: "relative", zIndex: 10
  }}>
    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
      <div style={{
        width: 28, height: 28, background: css.accent, borderRadius: 3,
        display: "flex", alignItems: "center", justifyContent: "center",
        fontSize: 13, color: "#fff", fontWeight: "bold", flexShrink: 0
      }}>⊞</div>
      <div>
        <div style={{ fontSize: isMobile ? 12 : 13, fontWeight: "bold", letterSpacing: "0.15em", color: css.textHi, textTransform: "uppercase" }}>ALPR Spotlight</div>
        {!isMobile && <div style={{ fontSize: 9, color: css.textLo, letterSpacing: "0.2em", textTransform: "uppercase" }}>Public Records Request Tool</div>}
      </div>
    </div>
    ...
    {/* Mobile step indicator + sheet toggle */}
    {isMobile && (
      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
        <div style={{ fontSize: 10, color: css.textLo, letterSpacing: "0.1em" }}>
          <span style={{ color: css.accent, fontWeight: "bold" }}>{step}</span>
    ...
    <div style={{ flex: 1, display: "flex", flexDirection: "column", position: "relative", overflow: "hidden" }}>
      ...
      {loadingAgencies && pin && (
        <div style={{
          ...
          <span style={{ display: "inline-block", width: 10, height: 10, border: `2px solid ${css.blue}`, borderTopColor: "transparent", borderRadius: "50%", animation: "spin 0.7s linear infinite" }} />
          <span style={{ fontSize: 10, color: css.blue, letterSpacing: "0.1em" }}>Querying database…</span>
        </div>
      )}
    </div>
  </div>
    ...
    <div style={{ position: "relative", height: "calc(100vh - 48px)", overflow: "hidden" }}>
      ...
      {loadingAgencies && pin && (
        <div style={{
          ...
          background: "rgba(6,9,15,0.92)", border: `1px solid ${css.blue}44`,
          borderRadius: 20, padding: "7px 16px", display: "flex", alignItems: "center", gap: 8, zIndex: 500
        }}>
          <span style={{ display: "inline-block", width: 9, height: 9, border: `2px solid ${css.blue}`, borderTopColor: "transparent", borderRadius: "50%", animation: "spin 0.7s linear infinite" }} />
          <span style={{ fontSize: 10, color: css.blue, letterSpacing: "0.1em" }}>Searching area…</span>
        </div>
      )}

      {/* Map hint when sheet is hidden */}
      {step === 1 && !pin && leafletReady && !sheetOpen && (
        <div style={{
          position: "absolute", bottom: 24, left: "50%", transform: "translateX(-50%)",
          background: "rgba(6,9,15,0.88)", border: `1px solid ${css.border}`,
          borderRadius: 6, padding: "9px 18px", fontSize: 11, color: css.textLo,
          backdropFilter: "blur(4px)", pointerEvents: "none", whiteSpace: "nowrap", zIndex: 400
        }}>
          ...
          <div style={{ display: "flex", gap: 6, marginTop: 6 }}>
            {step > 1 && (
              <button onClick={() => setStep(s => s - 1)} style={{
                background: css.blueLo, border: `1px solid ${css.blue}44`, borderRadius: 4,
                color: css.blue, padding: "5px 10px", cursor: "pointer", fontSize: 10
              }}>← Back</button>
            )}
          </div>
        </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "14px 16px 32px" }}>
          ...
          {step === 1 && (
            <div style={{ animation: "fadeIn 0.2s ease" }}>
              <div style={{ fontSize: 11, color: css.textLo, marginBottom: 14, lineHeight: 1.7 }}>
                Search an address or tap the map to drop a pin, then set your search radius.
              </div>
              <div style={{ display: "flex", gap: 6, marginBottom: 14 }}>
                <input value={addressInput} onChange={e => setAddressInput(e.target.value)}
                  onKeyDown={e => e.key === "Enter" && geocode()}
                  placeholder="Search address..."
                  style={{ flex: 1, background: css.card, border: `1px solid ${css.border}`, borderRadius: 4, color: css.textHi, padding: "10px 12px", fontSize: 14, outline: "none" }}
                />
                <button onClick={geocode} disabled={geocoding} style={{
                  background: css.blueLo, border: `1px solid ${css.blue}44`, borderRadius: 4,
                  color: css.blue, padding: "10px 14px", cursor: "pointer", fontSize: 12
                }}>{geocoding ? "..." : "GO"}</button>
              </div>
              <div style={{ fontSize: 11, color: css.textLo, marginBottom: 6 }}>
                Radius: <span style={{ color: css.textHi, fontWeight: "bold" }}>{radius} miles</span>
              </div>
              <input type="range" min="0.5" max="25" step="0.5" value={radius}
                onChange={e => setRadius(parseFloat(e.target.value))}
                style={{ width: "100%", accentColor: css.accent, marginBottom: 16 }}
              />
              {pin && !loadingAgencies && (
                <div style={{ background: css.card, border: `1px solid ${css.border}`, borderRadius: 6, padding: 14 }}>
                  <div style={{ fontSize: 13, color: css.textHi, marginBottom: agencies.length > 0 ? 10 : 0 }}>
                    <span style={{ color: css.accent, fontWeight: "bold", fontSize: 20 }}>{agencies.length}</span>
                    {" "}agencies found within {radius} mi
                  </div>
                  {agencies.length > 0 && (
                    <button onClick={() => setStep(2)} style={{
                      width: "100%", background: css.accent, border: "none", borderRadius: 4,
                      color: "#fff", padding: "11px", cursor: "pointer", fontSize: 13,
                      letterSpacing: "0.12em", textTransform: "uppercase"
                    }}>Continue →</button>
                  )}
                </div>
              )}
              {agencyError && <div style={{ fontSize: 11, color: css.accent, marginTop: 8 }}>{agencyError}</div>}
            </div>
          )}
          ...
          {step === 2 && (
            <div style={{ animation: "fadeIn 0.2s ease" }}>
              <div style={{ fontSize: 11, color: css.textLo, marginBottom: 12, lineHeight: 1.7 }}>
                Choose what records to request. All selections appear in every letter.
              </div>
              {REQUEST_TYPES.map(rt => {
                const sel = selectedTypes.includes(rt.id);
                return (
                  <div key={rt.id} onClick={() => toggleType(rt.id)} style={{
                    display: "flex", gap: 10, padding: "12px", marginBottom: 6, borderRadius: 6, cursor: "pointer",
                    background: sel ? "#0f1e30" : css.card, border: `1px solid ${sel ? css.borderHi : css.border}`
                  }}>
                    <div style={{
                      width: 18, height: 18, borderRadius: 4, flexShrink: 0, marginTop: 2,
                      background: sel ? css.accent : "transparent", border: `2px solid ${sel ? css.accent : css.textDim}`,
                      display: "flex", alignItems: "center", justifyContent: "center"
                    }}>
                      {sel && <span style={{ color: "#fff", fontSize: 11 }}>✓</span>}
                    </div>
                    <div>
                      <div style={{ fontSize: 13, color: sel ? css.textHi : css.textLo, fontWeight: sel ? "bold" : "normal" }}>{rt.icon} {rt.label}</div>
                      <div style={{ fontSize: 10, color: css.textDim, marginTop: 2, lineHeight: 1.5 }}>{rt.description}</div>
                    </div>
                  </div>
                );
              })}
              <div style={{ marginTop: 14, padding: 12, background: css.card, border: `1px solid ${css.border}`, borderRadius: 6 }}>
                <div style={{ fontSize: 10, color: css.textLo, letterSpacing: "0.12em", textTransform: "uppercase", marginBottom: 10 }}>Your Information (Optional)</div>
                <input value={requesterName} onChange={e => setRequesterName(e.target.value)} placeholder="Full name"
                  style={{ width: "100%", background: css.bg, border: `1px solid ${css.border}`, borderRadius: 3, color: css.textHi, padding: "10px", fontSize: 14, outline: "none", marginBottom: 8 }} />
                <input value={requesterEmail} onChange={e => setRequesterEmail(e.target.value)} placeholder="Your email"
                  style={{ width: "100%", background: css.bg, border: `1px solid ${css.border}`, borderRadius: 3, color: css.textHi, padding: "10px", fontSize: 14, outline: "none" }} />
              </div>
              <button onClick={() => setStep(3)} disabled={selectedTypes.length === 0} style={{
                marginTop: 14, width: "100%", background: selectedTypes.length > 0 ? css.accent : "#1a2535",
                border: "none", borderRadius: 4, color: selectedTypes.length > 0 ? "#fff" : css.textDim,
                padding: "12px", cursor: selectedTypes.length > 0 ? "pointer" : "not-allowed",
          ...
          {step === 3 && (
            <div style={{ animation: "fadeIn 0.2s ease" }}>
              <div style={{ fontSize: 11, color: css.textLo, marginBottom: 10, lineHeight: 1.7 }}>
                {agencies.length} agencies found. Looking up records emails automatically.
              </div>
              <div style={{ display: "flex", gap: 6, marginBottom: 12 }}>
                <button onClick={() => setSelectedAgencies(new Set(agencies.map(a => a.id)))} style={{
                  flex: 1, background: css.card, border: `1px solid ${css.border}`, borderRadius: 4,
                  color: css.blue, padding: "8px", cursor: "pointer", fontSize: 11
                }}>SELECT ALL</button>
                <button onClick={() => setSelectedAgencies(new Set())} style={{
                  flex: 1, background: css.card, border: `1px solid ${css.border}`, borderRadius: 4,
                  color: css.blue, padding: "8px", cursor: "pointer", fontSize: 11
                }}>DESELECT ALL</button>
              </div>
              {agencies.map(a => {
                ...
                return (
                  <div key={a.id} style={{
                    padding: "12px", marginBottom: 7, borderRadius: 6,
                    background: sel ? "#0d1a2a" : css.card, border: `1px solid ${sel ? css.borderHi : css.border}`
                  }}>
                    <div style={{ display: "flex", gap: 10 }}>
                      ...
                      <div style={{ flex: 1, cursor: "pointer" }} onClick={() => toggleAgency(a.id)}>
                        <div style={{ fontSize: 13, color: css.textHi, fontWeight: "bold", marginBottom: 3 }}>{a.name}</div>
                        <div style={{ fontSize: 10, color: css.textDim, marginBottom: 5 }}>{a.type}{a.state ? ` · ${a.state}` : ""}</div>
                        <StatusDot status={status} />
                        {(a.email || emailOverrides[a.id]) && (
                          <div style={{ fontSize: 10, color: css.textLo, marginTop: 3 }}>→ {emailOverrides[a.id] || a.email}</div>
                        )}
                      </div>
                    </div>
                    {sel && (status === "not_found" || status === "no_website" || status === "error") && (
                      <input placeholder="Enter email manually..."
                        value={emailOverrides[a.id] || ""}
                        onChange={e => setEmailOverrides(p => ({ ...p, [a.id]: e.target.value }))}
                        style={{ width: "100%", background: css.bg, border: `1px solid ${css.border}`, borderRadius: 3, color: css.textHi, padding: "9px 10px", fontSize: 13, outline: "none", marginTop: 10 }}
                      />
                    )}
                  </div>
                );
              })}
              <button onClick={() => setStep(4)} disabled={selectedAgencies.size === 0} style={{
                marginTop: 10, width: "100%", background: selectedAgencies.size > 0 ? css.accent : "#1a2535",
                border: "none", borderRadius: 4, color: selectedAgencies.size > 0 ? "#fff" : css.textDim,
                padding: "12px", cursor: selectedAgencies.size > 0 ? "pointer" : "not-allowed",
                fontSize: 13, letterSpacing: "0.12em", textTransform: "uppercase"
              }}>Review & Send →</button>
            </div>
          )}
          ...
          {step === 4 && (
            <div style={{ animation: "fadeIn 0.2s ease" }}>
              <div style={{ fontSize: 11, color: css.textLo, marginBottom: 12, lineHeight: 1.7 }}>
                Tap ✉ OPEN to launch each request in your email app, or send all at once.
              </div>
              {agencies.filter(a => selectedAgencies.has(a.id)).map(a => (
                <div key={a.id} style={{
                  display: "flex", alignItems: "center", justifyContent: "space-between",
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 13, color: css.textHi, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{a.name}</div>
                    <div style={{ fontSize: 10, color: getEmail(a) ? css.green : css.textDim }}>
                      {getEmail(a) || "No email found"}
                    </div>
                  </div>
                  {getEmail(a) ? (
                    <button onClick={() => openEmail(a)} style={{
                      background: css.blueLo, border: `1px solid ${css.blue}44`, borderRadius: 4,
                      color: css.blue, padding: "8px 14px", cursor: "pointer", fontSize: 12,
                    ...
                    <button onClick={() => {
                      ...
                      background: "#1a2a1a", border: `1px solid ${css.green}44`, borderRadius: 4,
                      color: css.green, padding: "8px 14px", cursor: "pointer", fontSize: 12,
              ...
              <button onClick={openAll} style={{
                width: "100%", background: css.accent, border: "none", borderRadius: 4,
                color: "#fff", padding: "13px", cursor: "pointer", fontSize: 13,
                letterSpacing: "0.12em", textTransform: "uppercase", marginTop: 12
              }}>✉ OPEN ALL IN EMAIL APP</button>
              <div style={{ fontSize: 10, color: css.textDim, textAlign: "center", marginTop: 8, lineHeight: 1.7 }}>
                Zero data sent through this tool.
              </div>

              {/* State law reference */}
              <div style={{ marginTop: 16, padding: 12, background: css.card, border: `1px solid ${css.border}`, borderRadius: 6 }}>
                <div style={{ fontSize: 10, color: css.textLo, letterSpacing: "0.12em", textTransform: "uppercase", marginBottom: 8 }}>📖 State Open Records Law</div>
                <select value={selectedState} onChange={e => setSelectedState(e.target.value)} style={{
                  width: "100%", background: css.bg, border: `1px solid ${css.borderHi}`, borderRadius: 4,
                  color: css.textHi, padding: "10px", fontSize: 14, outline: "none", cursor: "pointer", marginBottom: 8
                }}>
                  <option value="">— Select a state —</option>
                  {STATE_RECORDS_LAWS.map(s => <option key={s.code} value={s.code}>{s.name}</option>)}
                </select>
                {selectedState && (() => {
                  const entry = STATE_RECORDS_LAWS.find(s => s.code === selectedState);
                  return entry ? (
                    <a href={entry.url} target="_blank" rel="noopener noreferrer" style={{
                      display: "block", background: css.blueLo, border: `1px solid ${css.blue}44`,
                      borderRadius: 4, color: css.blue, padding: "10px", fontSize: 12,
                      textDecoration: "none", letterSpacing: "0.08em", textAlign: "center"
                    }}>SEARCH GOOGLE: {entry.law} →</a>
                  ) : null;
                })()}
              </div>
            </div>
